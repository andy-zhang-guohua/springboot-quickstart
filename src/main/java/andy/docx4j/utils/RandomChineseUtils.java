...
public class RandomChineseUtils {
    public static String randomChineseName() {
        ...
        int index = random.nextInt(familyNames.length - 1);
        String name = familyNames[index]; //获得一个随机的姓氏

        /* 从常用字中选取一个或两个字作为名 */
        if (random.nextBoolean()) {
            name += randomChineseChar() + randomChineseChar();
        } else {
    ...
    public static char randomChineseChar() {
        ...
        try {
            String str = new String(bytes, "GB2312");    //区位码组合成汉字
            return str.charAt(0);
